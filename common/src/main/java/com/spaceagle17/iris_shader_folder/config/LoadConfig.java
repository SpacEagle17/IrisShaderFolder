...
public class LoadConfig {
    ...
    public static void loadConfigOptions() {
        boolean configDebugLog = Boolean.parseBoolean(ConfigManager.readWriteConfig(
                "debugLogging",
                "false",
                "Enable debug logging (creates a debug file with detailed pattern processing)"
        ));
        handleJVMArgumentDebugLogging(configDebugLog);
    ...
    private static void handleJVMArgumentDebugLogging(boolean configDebugLogging) {
        // Check for JVM argument -DebugIrisShaderFolder=true/false which takes priority over config
        String jvmDebugArg = System.getProperty("ebugIrisShaderFolder");
        if (jvmDebugArg != null) {
            String argLower = jvmDebugArg.trim().toLowerCase(Locale.ROOT);
            if ("true".equals(argLower) || "false".equals(argLower)) {
                debugLoggingEnabled = Boolean.parseBoolean(argLower);
                debugLog("Debug logging set via JVM argument -DebugIrisShaderFolder=" + jvmDebugArg + " (overriding config value)");
            } else {
                IrisShaderFolder.log(2, "Invalid value for -DebugIrisShaderFolder: " + jvmDebugArg + ". Only 'true' or 'false' are accepted. Using config value.");
                debugLoggingEnabled = configDebugLogging;
            }
        } else {
            debugLoggingEnabled = configDebugLogging;
        }
    ...
    private static void debugLog(String message) {
        if (debugLoggingEnabled) {
            IrisShaderFolder.debugLog("[LoadConfig]" + message);
        }
    }
